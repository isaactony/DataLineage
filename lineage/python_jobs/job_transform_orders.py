...
class OrderTransformJob:
    ...
    def __init__(self, marquez_url: str = None):
        ...
        self.db_url = os.getenv("DATABASE_URL", "postgresql://marquez:marquez@postgres:5432/marquez")
        ...
        outputs = [
            {
                ...
                "schema": [
                    {"name": "order_id", "type": "integer", "description": "Unique order identifier"},
                    {"name": "customer_id", "type": "integer", "description": "Customer identifier"},
                    {"name": "customer_name", "type": "varchar", "description": "Full customer name"},
                    {"name": "customer_email", "type": "varchar", "description": "Customer email"},
                    {"name": "order_date", "type": "date", "description": "Order date"},
                    {"name": "amount", "type": "decimal", "description": "Order amount"},
                    {"name": "status", "type": "varchar", "description": "Order status"},
                    {"name": "order_month", "type": "varchar", "description": "Order month"},
            ...
            },
            {
                ...
                "schema": [
                    ...
                    {"name": "last_order_date", "type": "date", "description": "Last order date"},
                    {"name": "processed_at", "type": "timestamp", "description": "Processing timestamp"}
                ],
                "data_source": "postgresql"
            }
    ...
    def _enrich_orders(self, orders_df: pd.DataFrame, customers_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Add derived fields
        enriched_df['customer_name'] = enriched_df['first_name'] + ' ' + enriched_df['last_name']
        enriched_df['customer_email'] = enriched_df['email']
        enriched_df['order_month'] = pd.to_datetime(enriched_df['order_date']).dt.to_period('M').astype(str)
        enriched_df['processed_at'] = datetime.now()
        
        # Select final columns
        result_df = enriched_df[[
            'order_id', 'customer_id', 'customer_name', 'customer_email',
            'order_date', 'amount', 'status', 'order_month', 'processed_at'
        ]]
    ...
    def _create_order_summary(self, enriched_orders_df: pd.DataFrame) -> pd.DataFrame:
        ...
        summary_df = enriched_orders_df.groupby(['customer_id', 'customer_name']).agg({
            'order_id': 'count',
            'amount': ['sum', 'mean'],
            'order_date': 'max'
        }).reset_index()
        ...
        summary_df.columns = [
            ...
            'total_amount', 'avg_order_value', 'last_order_date'
        ]
